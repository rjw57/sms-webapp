# Settings which are specific to deployment
import os

# Start from base settings
from smswebapp.settings import *  # noqa: F401, F403

# Retrieve the secret key from the environment.
SECRET_KEY = os.environ['DJANGO_SECRET_KEY']

# Debug-related functionality is only enabled if the SMS_ENABLE_DEBUG_THIS_IS_DANGEROUS environment
# variable is set and is non-empty.
DEBUG = os.environ.get('SMS_ENABLE_DEBUG_THIS_IS_DANGEROUS', '') != ''

# Allow all hosts to connect
ALLOWED_HOSTS = ['*']

# Static files configuration. The STATIC_URL should point to the appropriate Google Storage
# location for the webapp's branch.
STATIC_ROOT = 'static/'
STATIC_URL = os.environ.get('STATIC_URL', '')

# Raven configuration for ucamwebauth
UCAMWEBAUTH_LOGIN_URL = 'https://raven.cam.ac.uk/auth/authenticate.html'
UCAMWEBAUTH_LOGOUT_URL = 'https://raven.cam.ac.uk/auth/logout.html'
UCAMWEBAUTH_CERTS = {2: """
-----BEGIN CERTIFICATE-----

hAM+a6/30F5fdkWpE1smPyrfASyXRfWE4Ccn1RVgYX9u
-----END CERTIFICATE-----
"""}

# Logging
LOGGING = {
    'version': 1,
    'disable_existing_loggers': False,
    'formatters': {
        'verbose': {
            'format': '%(levelname)s %(asctime)s %(pathname)s:%(lineno)d %(funcName)s "%(message)s"'
        },
        'simple': {
            'format': '%(levelname)s "%(message)s"'
        },
    },
    'handlers': {
        'console': {
            'level': 'INFO',
            'class': 'logging.StreamHandler',
            'formatter': 'verbose',
        },
    },
    'loggers': {
        'django': {
            'handlers': ['console'],
            'propagate': True,
            'level': 'INFO'
        },
    }
}


# This is not a secret per se since it is exposed in every embedding of the player, but it *is*
# specific to our deployment
JWPLATFORM_EMBED_PLAYER_KEY = os.environ['JWPLATFORM_EMBED_PLAYER_KEY']
